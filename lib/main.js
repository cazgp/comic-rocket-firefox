...
panel.port.on("not-logged-in", function() {
});
...
var get_unread = function(no_notify) {
  ...
  var req = Request({
    ...
    onComplete: function (response) {
      ...
      timers.setTimeout(get_unread, timeout*60000);

      // Only notify if not initial
      if(!no_notify) {
        var number_new_entries = comic_rocket.count_new(comics);
